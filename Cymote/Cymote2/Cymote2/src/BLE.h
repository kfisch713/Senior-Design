...
#define MAX_NUM_CHARACTERISTICS 4
...
typedef struct {
	...
	uint8_t accel_x[ACCEL_X_MAX_LEN];
...
#define UPDATE_ACCEL_X(ptr, info_data, conn_handle) do {   \
	if (cymote_info_update(ptr, CYMOTE_ACCEL_X, info_data, conn_handle) != AT_BLE_SUCCESS) { \
